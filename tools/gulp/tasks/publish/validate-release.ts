...
import {buildConfig, sequenceTask} from 'lernender-angular-build-tools';
...
function checkEs2015ReleaseBundle(packageName: string, bundlePath: string): string[] {
  ...
  if (packageName === 'common') {
    failures = failures.concat(checkLibPackage());
  }

  return failures;
}

/** Function that includes special checks for the Angular package. */
function checkLibPackage(): string[] {
  const packagePath = join(releasesDir, 'common');
  ...
  if (glob('*.css', {cwd: prebuiltThemesPath}).length === 0) {
    failures.push('Prebuilt themes are not present in the Angular release output.');
  }

  if (!existsSync(themingFilePath)) {
    failures.push('The theming SCSS file is not present in the Angular release output.');
  }
