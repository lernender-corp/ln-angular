...
import {buildConfig, buildScssPipeline, sequenceTask} from 'lernender-angular-build-tools';
...
task('prerender', ['universal:build'], execTask(
  // Runs node with the tsconfig-paths module to alias the @lernender/angular dependency.
...
task('universal:build', sequenceTask(
  'clean',
  ['angular:build-release', 'cdk:build-release'],
...
// copy the Angular and CDK ESM output inside of the universal-app output.
task('universal:copy-release', () => {
  copySync(join(releasesDir, 'common'), join(outDir, 'common'));
});
