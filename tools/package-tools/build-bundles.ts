...
export class PackageBundler {
  ...
  /** Bundles the primary entry-point w/ given entry file, e.g. @lernender/cdk */
  private async _bundlePrimaryEntryPoint() {
    ...
    return this._bundleEntryPoint({
      ...
      importName: `@lernender/${packageName}`,
  ...
  /** Bundles a single secondary entry-point w/ given entry file, e.g. @lernender/cdk/a11y */
  private async _bundleSecondaryEntryPoint(entryPointName: string) {
    ...
    return this._bundleEntryPoint({
      entryFile,
      esm5EntryFile,
      importName: `@lernender/${packageName}/${entryPointName}`,
  ...
  private async _createRollupBundle(config: RollupBundleConfig) {
    ...
    const writeOptions = {
      name: config.moduleName || 'ng.lernender',
    ...
    if (config.format === 'umd') {
      ...
      if (this._buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        const importRegex = new RegExp(`@lernender/${this._buildPackage.name}/.+`);
  ...
  /**
   * Gets mapping of import aliases (e.g. `@lernender/angular/button`) to the path of the es5
   * bundle output.
  ...
  private _getResolvedSecondaryEntryPointImportPaths(bundleOutputDir: string) {
    return this._buildPackage.secondaryEntryPoints.reduce((map, p) => {
      map[`@lernender/${this._buildPackage.name}/${p}`] =
