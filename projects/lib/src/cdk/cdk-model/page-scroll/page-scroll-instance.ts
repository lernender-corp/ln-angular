...
import { PageScrollEasingLogic } from './page-scroll-easing-logic';
...
export interface PageScrollOptions extends PageScrollConfig {
  ...
  easingLogic?: PageScrollEasingLogic;
}
...
export class PageScrollInstance {
  ...
  private static getScrollingTargetPosition(
    ...
    const windowPageYOffset: number =
      (pageScrollOptions.document.defaultView &&
        pageScrollOptions.document.defaultView.pageYOffset) ||
      undefined;
    const windowPageXOffset: number =
      (pageScrollOptions.document.defaultView &&
        pageScrollOptions.document.defaultView.pageXOffset) ||
      undefined;
    ...
    if (scrollTargetElement === undefined || scrollTargetElement === null) {
      // No element found, so return the current position to not
      // cause any change in scroll position
  ...
  private static getInlineScrollingTargetPosition(
    ...
    if (
      pageScrollOptions.advancedInlineOffsetCalculation &&
      pageScrollOptions.scrollViews.length === 1
    ) {
      ...
      while (!parentFound && parent !== undefined && parent !== null) {
        if (
          theWindow.getComputedStyle(parent).getPropertyValue('position') ===
          'relative'
        ) {
      ...
      if (parentFound) {
        ...
        // (PageScrollConfig._logLevel >= 1 && isDevMode())) {
  ...
  /* The target value of the scrollTop or scrollLeft position for the animation
    (aka "the final destination") */
  public targetScrollPosition: number;
  /* Difference between startScrollPosition and targetScrollPosition.
    Pre-calculated to minimize computations during animation */
  ...
  /* The duration a started animation takes. This may match the _duration or
    be adjusted due to the _speed option */
  ...
  constructor(pageScrollOptions?: any) {
    if (
      !pageScrollOptions.scrollViews ||
      pageScrollOptions.scrollViews.length === 0
    ) {
      pageScrollOptions.scrollViews = [
   ...
   *
   * Extract the scrollTarget HTMLElement from the given PageScrollTarget object.
   * The latter one may be
   * a string like "#heading2", then this method returns the corresponding
   * DOM element for that id.
   *
   */
  public extractScrollTargetPosition(): { top: number; left: number } {
    ...
    if (this.isInlineScrolling) {
      return PageScrollInstance.getInlineScrollingTargetPosition(
        this.pageScrollOptions,
        scrollTargetElement
      );
    }

    return PageScrollInstance.getScrollingTargetPosition(
      this.pageScrollOptions,
      scrollTargetElement
    );
  }
   ...
   * @return true if at least for one ScrollTopSource the scrollTop/scrollLeft value
   * could be set and it kept the new value.
  ...
  public setScrollPosition(position: number): boolean {
    ...
    return this.pageScrollOptions.scrollViews.reduce(
      (oneAlreadyWorked: any, scrollingView: any) => {
        const startScrollPropertyValue = this.getScrollPropertyValue(
          scrollingView
        );
        if (
          scrollingView &&
          startScrollPropertyValue !== undefined &&
          startScrollPropertyValue !== null
        ) {
          ...
          const isSmallMovement =
            scrollDistance < this.pageScrollOptions._minScrollDistance;
          ...
          if (
            isSmallMovement ||
            scrollDistance >
              Math.abs(this.getScrollPropertyValue(scrollingView) - position)
          ) {
            return true;
          }
        }
   ...
   *
   * Possibly attached interruptListeners are automatically removed from the body
   * before the new one will be attached.
   */
  public attachInterruptListeners(interruptReporter: InterruptReporter): void {
    ...
    this.pageScrollOptions.interruptEvents.forEach((event: string) =>
  ...
  /**
   * Remove event listeners from the body and stop listening for events that
   * might be treated as "animation
   * interrupt" events.
   */
  ...
  private getScrollTargetElement(): HTMLElement {
    if (typeof this.pageScrollOptions.scrollTarget === 'string') {
      ...
      if (targetSelector.match(/^#[^\s]+$/g) !== null) {
        ...
        return this.pageScrollOptions.document.getElementById(
          targetSelector.substr(1)
        );
      }
