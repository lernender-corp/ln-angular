...
export class LnPageScrollService {
  ...
  private onInterrupted: InterruptReporter = {
    report: (event: Event, pageScrollInstance: PageScrollInstance): void => {
      ...
      if (event.type === 'keyup') {
        ...
        if (
          this.config.interruptKeys.indexOf((<KeyboardEvent>event).key) === -1
        ) {
        ...
        if (
          !pageScrollInstance.pageScrollOptions.scrollViews.some(
            (scrollingView: any) => scrollingView.contains(event.target as Node)
          )
        ) {
  ...
  /**
   * Start a scroll animation. All properties of the animation are
  ...
  public start(pageScrollInstance: PageScrollInstance): void {
    ...
    if (
      pageScrollInstance.pageScrollOptions.scrollViews === null ||
      pageScrollInstance.pageScrollOptions.scrollViews.length === 0
    ) {
      // No scrollViews specified, thus we can't animate anything
      if (
        this.config._logLevel >= 2 ||
        (this.config._logLevel >= 1 && isDevMode())
      ) {
        console.warn(
          `No scrollViews specified, thus ngx-page-scroll
					does not know which DOM elements to scroll`
        );
      }
    ...
    // Reset start scroll position to 0. If any of the scrollViews
    // has a different one, it will be extracted next
    pageScrollInstance.startScrollPosition = 0;

    // Get the start scroll position from the scrollViews
    // (e.g. if the user already scrolled down the content)
    pageScrollInstance.pageScrollOptions.scrollViews.forEach(
      (scrollingView: any) => {
        if (scrollingView === undefined || scrollingView === null) {
          return;
        }
        // Get the scrollTop or scrollLeft value of the first
        // scrollingView that returns a value for its 'scrollTop'
        // or 'scrollLeft' property that is not undefined and unequal to 0

        const scrollPosition = pageScrollInstance.getScrollPropertyValue(
          scrollingView
        );
    ...
    pageScrollInstance.targetScrollPosition = Math.round(
      (pageScrollInstance.pageScrollOptions.verticalScrolling
    ...
    pageScrollInstance.distanceToScroll =
      pageScrollInstance.targetScrollPosition -
      pageScrollInstance.startScrollPosition;
    ...
    const allReadyAtDestination =
      Math.abs(pageScrollInstance.distanceToScroll) <
      pageScrollInstance.pageScrollOptions._minScrollDistance;

    // Check how long we need to scroll if a speed option is given
    // Default executionDuration is the specified duration
    pageScrollInstance.executionDuration =
      pageScrollInstance.pageScrollOptions.duration;
    // Maybe we need to pay attention to the speed option?
    if (
      pageScrollInstance.pageScrollOptions.speed !== undefined &&
      pageScrollInstance.pageScrollOptions.speed !== null &&
      (pageScrollInstance.pageScrollOptions.duration === undefined ||
        pageScrollInstance.pageScrollOptions.duration === null)
    ) {
      // Speed option is set and no duration =>
      // calculate duration based on speed and scroll distance
      pageScrollInstance.executionDuration =
        (Math.abs(pageScrollInstance.distanceToScroll) /
          pageScrollInstance.pageScrollOptions.speed) *
        1000;
    }

    // We should go there directly, as our 'animation' would have one big step
    // only anyway and this way we save the interval stuff
    const tooShortInterval =
      pageScrollInstance.executionDuration <=
      pageScrollInstance.pageScrollOptions._interval;
    ...
    if (allReadyAtDestination || tooShortInterval) {
      ...
      pageScrollInstance.setScrollPosition(
        pageScrollInstance.targetScrollPosition
      );
      ...
      (_pageScrollInstance: PageScrollInstance) => {
        ...
        if (_pageScrollInstance.endTime <= currentTime) {
          ...
          newScrollPosition = Math.round(
            _pageScrollInstance.pageScrollOptions.easingLogic(
  ...
  /**
   * Stop all running scroll animations. Optionally limit
   * to stop only the ones of specific namespace.
   */
  public stopAll(namespace?: string): boolean {
    if (this.runningInstances.length > 0) {
      ...
      for (let i = 0; i < this.runningInstances.length; ++i) {
        ...
        if (
          !namespace ||
          pageScrollInstance.pageScrollOptions.namespace === namespace
        ) {
          stoppedSome = true;
  ...
  constructor(@Inject(NGXPS_CONFIG) customConfig: PageScrollConfig) {
    ...
    if (
      LnPageScrollService.instanceCounter > 0 &&
      (this.config._logLevel >= 2 ||
        (this.config._logLevel >= 1 && isDevMode()))
    ) {
      console.warn(
    ...
    LnPageScrollService.instanceCounter++;
  }
}
